...
class TestCifra(unittest.TestCase):
	...
	def test_chave_maior_que_o_texto_base_retornar_exception(self):
		...
		"""
		c = Cifra()
		assert_raises(ValueError, c.crypt, 'a', 'bb')
	...
	def test_normaliza_tamanho_LIMAO(self):
		...
		"""
		c = Cifra()
		self.assertEquals(c.match_size('ATACARBASESUL', 'LIMAO'), 'LIMAOLIMAOLIM')

	def test_normaliza_tamanho_LIMA(self):
		"""
			A chave LIMA com um texto de 13 caracteres o retorno deve ser LIMALIMALIMAL
		"""
		c = Cifra()
		self.assertEquals(c.match_size('ATACARBASESUL', 'LIMA'), 'LIMALIMALIMAL' )

	def test_ATACARBASESUL_chave_LIMAO_retorno_LBMCOCJMSSDCX(self):
		"""
			A criptografia de ATACARBASESUL com a chave LIMAO deve ser LBMCOCJMSSDCX
		"""
		c = Cifra()
		self.assertEquals(c.crypt('ATACARBASESUL', 'LIMAO').upper(), 'LBMCOCJMSSDCX')
